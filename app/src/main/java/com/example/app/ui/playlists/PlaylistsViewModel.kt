...
class PlaylistsViewModel(private val repository: PlaylistRepository) : ViewModel() {
    ...
    // Базова LiveData зі списком тільки плейлистів
     val _allPlaylists: LiveData<List<Playlist>> = repository.allPlaylists
    ...
    suspend fun getSoundsForPlaylist(playlistId: Int): List<Sound> {
        return repository.getSoundsForPlaylist(playlistId)
    }
    suspend fun getPlaylistById(playlistId: Int): Playlist? {
        return repository.getPlaylistById(playlistId)
    }
    suspend fun getSoundByID(soundId: String): Sound? {
        return repository.getSoundById(soundId)
    }
